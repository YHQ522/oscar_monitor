...
The bundled scripts require Python 3 (standard library only 鈥?no third-party packages, no network access). Check if it is available:
...
> **Note:** On Windows, use `python` instead of `python3` to run scripts (e.g., `python scripts/search.py` instead of `python scripts/search.py`).
...
|----------|-----------------|------------|
| **New project / page** | "鍋氫竴涓?landing page"銆?Build a dashboard" | Step 1 鈫?Step 2 (design system) |
| **New component** | "Create a pricing card"銆?Add a modal" | Step 3 (domain search: style, ux) |
| **Choose style / color / font** | "What style fits a fintech app?"銆?鎺ㄨ崘閰嶈壊" | Step 2 (design system) |
| **Review existing UI** | "Review this page for UX issues"銆?妫€鏌ユ棤闅滅" | Quick Reference checklist above |
| **Fix a UI bug** | "Button hover is broken"銆?Layout shifts on load" | Quick Reference 鈫?relevant section |
| **Improve / optimize** | "Make this faster"銆?Improve mobile experience" | Step 3 (domain search: ux, react) |
| **Implement dark mode** | "Add dark mode support" | Step 3 (domain: style "dark mode") |
| **Add charts / data viz** | "Add an analytics dashboard chart" | Step 3 (domain: chart) |
| **Stack best practices** | "React performance tips"銆?SwiftUI navigation" | Step 4 (stack search) |
...
- **Stack**: whatever the user is actually building with 鈥?infer it from the project
...
- **Platform**: web or native app. Several sections below are scoped to App UI
  (iOS/Android/React Native/Flutter) and do not apply to desktop-web work 鈥?
...
- `--density` overrides the `--space-*` CSS variable table in the ASCII/markdown/MASTER.md output 鈥?use it for dashboards (high) vs. marketing pages (low) without hand-editing tokens.
...
```bash
python .github/prompts/ui-ux-pro-max/scripts/search.py "atlantafx primer enterprise theme" --stack javafx
python .github/prompts/ui-ux-pro-max/scripts/search.py "enterprise tableview density permission" --stack javafx
```
...
**User request:** "Make an AI search homepage銆?
...
```bash
python .github/prompts/ui-ux-pro-max/scripts/search.py "AI search tool modern minimal" --design-system -p "AI Search"
```
...
python .github/prompts/ui-ux-pro-max/scripts/search.py "minimalism dark mode" --domain style

# Get UX best practices for search interaction and loading
python .github/prompts/ui-ux-pro-max/scripts/search.py "search loading animation" --domain ux
```

### Step 4: Stack Guidelines

```bash
python .github/prompts/ui-ux-pro-max/scripts/search.py "list performance navigation" --stack react-native
```
...
- Use **multi-dimensional keywords** 鈥?combine product + industry + tone + density: `"entertainment social vibrant content-dense"` not just `"app"`
- Try different keywords for the same need: `"playful neon"` 鈫?`"vibrant dark"` 鈫?`"content-first minimal"`
...
| Dark mode contrast issues | Quick Reference 搂6: `color-dark-mode` + `color-accessible-pairs` |
| Animations feel unnatural | Quick Reference 搂7: `spring-physics` + `easing` + `exit-faster-than-enter` |
| Form UX is poor | Quick Reference 搂8: `inline-validation` + `error-clarity` + `focus-management` |
| Navigation feels confusing | Quick Reference 搂9: `nav-hierarchy` + `bottom-nav-limit` + `back-behavior` |
| Layout breaks on small screens | Quick Reference 搂5: `mobile-first` + `breakpoint-consistency` |
| Performance / jank | Quick Reference 搂3: `virtualize-lists` + `main-thread-budget` + `debounce-throttle` |

### Pre-Delivery Checklist

- Run `--domain ux "animation accessibility z-index loading"` as a UX validation pass before implementation
- Run through Quick Reference **搂1鈥撀?** (CRITICAL + HIGH) as a final review
- Test on 375px (small phone) and landscape orientation
- Verify behavior with **reduced-motion** enabled and **Dynamic Type** at largest size
- Check dark mode contrast independently (don't assume light mode values work)
- Confirm all touch targets 鈮?4pt and no content hidden behind safe areas
...
- 榛樿鍥炬爣搴撲娇鐢?**Phosphor (`@phosphor-icons/react`)**銆俙src/ui-ux-pro-max/data/icons.csv` 涓垪鍑虹殑鍙槸甯哥敤鎺ㄨ崘鍥炬爣锛屼笉鏄畬鏁撮泦鍚堛€?
- 褰撴帹鑽愯〃涓壘涓嶅埌鍚堥€傜殑鍥炬爣鏃讹細
  - **浼樺厛缁х画浠?Phosphor 鐨勫畬鏁村浘鏍囬泦涓€夋嫨浠讳綍璇箟鏇磋创鍒囩殑鍥炬爣**锛?
  - 濡傛灉 Phosphor 涔熸病鏈夌悊鎯抽€夐」锛屽彲浠ヤ娇鐢?**Heroicons (`@heroicons/react`)** 浣滀负澶囬€夛紝娉ㄦ剰淇濇寔椋庢牸涓€鑷达紙绾挎€?濉厖銆佺瑪鐢荤矖缁嗐€佸渾瑙掗鏍硷級銆?

| Rule | Standard | Avoid | Why It Matters |
|------|----------|--------|----------------|
| **No Emoji as Structural Icons** | Use vector-based icons (e.g., Phosphor `@phosphor-icons/react`, Heroicons `@heroicons/react`, react-native-vector-icons, @expo/vector-icons). | Using emojis (馃帹 馃殌 鈿欙笍) for navigation, settings, or system controls. | Emojis are font-dependent, inconsistent across platforms, and cannot be controlled via design tokens. |
...
| **Touch Target Minimum** | Minimum 44脳44pt interactive area (use hitSlop if icon is smaller). | Small icons without expanded tap area. | Meets accessibility and platform usability standards. |
